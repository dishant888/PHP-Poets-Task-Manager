...
require('header.php');
 ...
 <div class="container">
	...
	<div class="modal-dialog shadow">
 	...
 	<table class="table text-center table-striped border" id="content">
 		...
 		<tbody><!-- 
 			...
 			<?php $maini=0;$f=0;
 			$detailes=mysqli_query($con,"SELECT `task_rows`.`id`,`tasks`.`project_id`,`task_rows`.`spent_hrs`,`task_rows`.`created_on`,`task_rows`.`team_id`,`task_rows`.`task_id`,`task_rows`.`title`,`task_rows`.`status` FROM `tasks` INNER JOIN `task_rows` ON `tasks`.`id` = `task_rows`.`task_id` WHERE team_id=".$_SESSION['u_id']." and task_rows.delete_status=0 and project_id=$id and status='completed' order by id desc");
	...
	<div class="modal-dialog modal-xl shadow">
 	  ...
 	  <?php }if($f==0){echo "<td colspan='6' class='text-danger'><h2>No completed tasks</h2></td></tr>";} ?>
 		</tbody>
 	</table>
