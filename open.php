...
require('header.php');
 			...
 			<tr>
 				<th><form class="form-inline">
 					...
 					<select id="change_project" class="form-control ml-3">
 						<?php 
 						$dash=mysqli_query($con,"SELECT `tasks`.`project_id`,`tasks`.`delete_status`,`tasks`.`id`,`task_rows`.`task_id`,`task_rows`.`title`,`task_rows`.`status` FROM `tasks` INNER JOIN `task_rows` ON `tasks`.`id` = `task_rows`.`task_id` where team_id=".$_SESSION['u_id']." group by `tasks`.`project_id` order by project_id desc");
 				...
 				<th>
 					...
 					<label id="t_h" class="font-weight-normal"></label>
 				</th>
 				<th>
	...
	<div class="modal-dialog shadow">
			...
			<div class="modal-body">
					<form class="form-group" action="add_own_task.php" method="post">
						<div class="container">
							...
							<br>
						<div class="col-12">
							<label><b>Estimated hrs:</b></label>
							<input type="number" step="0.01" name="estimated" class="form-control" required>
						</div><br>
						<div class="text-center">
						<input type="submit" name="add_own_task" value="ADD" class="btn btn-primary w-25 form-control">
					    <button class="btn btn-danger" data-dismiss="modal">Close</button></div>
					</div>
					</form>
				</div>
		</div>
 	...
 	<table class="table text-center table-striped border" id="content">
 		...
 		<tbody><!-- 
 			...
 			<?php $maini=0;$f=0;
 			$detailes=mysqli_query($con,"SELECT `task_rows`.`id`,`tasks`.`project_id`,`task_rows`.`spent_hrs`,`task_rows`.`created_on`,`task_rows`.`team_id`,`task_rows`.`task_id`,`task_rows`.`title`,`task_rows`.`status` FROM `tasks` INNER JOIN `task_rows` ON `tasks`.`id` = `task_rows`.`task_id` WHERE team_id=".$_SESSION['u_id']." and task_rows.delete_status=0 and project_id=$id and status='open' order by id desc");
	...
	<div class="modal-dialog modal-xl shadow">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<input type="number" step="0.01" name="hours" value="00:00" class="form-control" required>
				</div>
				<!-- <div class="col-6">
			...
			<div class="col-8">
				<div class="container-fluid text-center">
					...
					<div class="cust">
				<?php 
				$history=mysqli_query($con,"select * from history where team_id=".$_SESSION['u_id']." and task_row_id=".$get_detailes['id']);$i=0;
 	  ...
 	  <?php }if($f==0){echo "<td colspan='6' class='text-danger'><h2>No open tasks</h2></td></tr>";} ?>
 		</tbody>
 	</table>
 ...
 <script type="text/javascript">
 	$(document).ready(function() {
 		...
 		else{i=parseInt($(this).find('td:nth-child(3)').text());}t=t+i;});
 		...
 		$('#get_status').on('change', function(event) {
 			...
 			location.href='project_detailes.php?id='+pro_id;
 			$.ajax({
 				url: 'filter.php',
 				type: 'GET',
 				data: {request: 'all' , id: pro_id},
 				beforeSend:function(){
 			...
 			var pro_id = $('#project_id').val();
 			location.href='in_progress.php?id='+pro_id;
 			$.ajax({
 				url: 'filter.php',
 				type: 'GET',
 				data: {request: 'in_progress' , id: pro_id},
 				beforeSend:function(){
 					$('#content').html("<h1 class='text-center'>Working on...!</h1>");
 				},
 				success:function(data){
 			...
 			{
 			var pro_id = $('#project_id').val();
 			location.href='completed.php?id='+pro_id;
 			$.ajax({
 				url: 'filter.php',
 				type: 'GET',
 				data: {request: 'completed' , id: pro_id},
 				beforeSend:function(){
 					$('#content').html("<h1 class='text-center'>Working on...!</h1>");
 				},
 				success:function(data){
